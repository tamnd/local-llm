...
.\scripts\03-ollama.ps1        # Ollama 0.32.9 -> OllamaServe task, loopback 11434
.\scripts\04-llamacpp.ps1      # llama.cpp b10362 CUDA build -> C:\llama.cpp
...
`Register-Service -AsSystem` registers the task under SYSTEM instead of gopher,
which is what makes it start at boot: an Interactive principal needs a window
station, so its AtStartup trigger never fires and the service only appears once
someone logs in. OllamaServe uses it; TabbyServe and GatewayServe still run
interactively and come up At-LogOn, because SYSTEM changes `%USERPROFILE%` and
neither has been checked against that yet.
